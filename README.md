<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:ffcccc,30:ff9999,60:ff6666,100:ff4d4d&height=100&section=header&text=&fontSize=0" width="100%"/>

<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.demolab.com?font=Lobster&color=333333&size=35&pause=1000&center=true&vCenter=true&random=false&width=435&lines=Hello%2C+I'm+IQ;Nice+To+Meet+You" alt="Typing SVG" />
</a>

<br/>

> ***"I question, therefore I am."***

</div>

<br/>

## <img src="https://api.iconify.design/lucide/sparkles.svg?color=%231a1a1a" width="28"/> IQ Universe

<table>
<tr>
<td align="center" width="33%">
<a href="https://iq-proof.github.io">
  <img src="https://api.iconify.design/lucide/book-text.svg?color=%2300d9ff" width="28"/>
  <br/>
  <b>IQ Lab Blog</b>
</a>
<br/>
<sub>딥다이브 발행 허브</sub>
<br/>
<br/>
<a href="https://iq-proof.github.io">
  <img src="https://img.shields.io/badge/iq--proof.github.io-00d9ff?style=flat-square&logo=astro&logoColor=white" />
</a>
</td>
<td align="center" width="33%">
<a href="https://iq-dev-lab.github.io">
  <img src="https://api.iconify.design/lucide/server-cog.svg?color=%23ff4d4d" width="28"/>
  <br/>
  <b>IQ Dev Lab</b>
</a>
<br/>
<sub>백엔드 deep-dive</sub>
<br/>
<br/>
<a href="https://iq-dev-lab.github.io">
  <img src="https://img.shields.io/badge/iq--dev--lab.github.io-ff4d4d?style=flat-square&logo=gitbook&logoColor=white" />
</a>
</td>
<td align="center" width="33%">
<a href="https://iq-ai-lab.github.io">
  <img src="https://api.iconify.design/lucide/brain-circuit.svg?color=%23a78bfa" width="28"/>
  <br/>
  <b>IQ AI Lab</b>
</a>
<br/>
<sub>AI 이론 deep-dive</sub>
<br/>
<br/>
<a href="https://iq-ai-lab.github.io">
  <img src="https://img.shields.io/badge/iq--ai--lab.github.io-a78bfa?style=flat-square&logo=openai&logoColor=white" />
</a>
</td>
</tr>
<tr>
<td align="center" width="33%">
<a href="https://iq-agent-lab.github.io">
  <img src="https://api.iconify.design/lucide/bot.svg?color=%23cc785c" width="28"/>
  <br/>
  <b>IQ Agent Lab</b>
</a>
<br/>
<sub>에이전트 자동화 인프라</sub>
<br/>
<br/>
<a href="https://iq-agent-lab.github.io">
  <img src="https://img.shields.io/badge/iq--agent--lab.github.io-cc785c?style=flat-square&logo=anthropic&logoColor=white" />
</a>
</td>
<td align="center" width="33%">
<a href="https://github.com/Woowa-8th-BE-IQ">
  <img src="https://api.iconify.design/lucide/orbit.svg?color=%2300bfa5" width="28"/>
  <br/>
  <b>Woowa&nbsp;8th&nbsp;BE</b>
</a>
<br/>
<sub>우테코 8기 백엔드</sub>
<br/>
<br/>
<a href="https://github.com/Woowa-8th-BE-IQ">
  <img src="https://img.shields.io/badge/Organization-00bfa5?style=flat-square&logo=github&logoColor=white" />
</a>
</td>
<td align="center" width="33%">
<a href="https://www.youtube.com/@SeeWhy-l3n">
  <img src="https://api.iconify.design/lucide/youtube.svg?color=%23FF0000" width="28"/>
  <br/>
  <b>See Why</b>
</a>
<br/>
<sub>수학·AI 시각화 채널</sub>
<br/>
<br/>
<a href="https://www.youtube.com/@SeeWhy-l3n">
  <img src="https://img.shields.io/badge/YouTube-FF0000?style=flat-square&logo=youtube&logoColor=white" />
</a>
</td>
</tr>
</table>

<br/>

## <img src="https://api.iconify.design/lucide/telescope.svg?color=%231a1a1a" width="28"/> Details

<details>
<summary>&nbsp;<img src="https://api.iconify.design/lucide/book-text.svg?color=%2300d9ff" width="16" align="center"/> &nbsp;<b>IQ Lab Blog</b> &nbsp;—&nbsp; 본질을 증명하는 딥다이브 발행 허브</summary>

<br/>

> ***"Not usage, but essence — recorded as proof."***

표면적인 사용법이 아닌, 본질을 증명하는 기록.

백엔드 시스템과 AI의 수학적 기반을 파고드는 딥다이브 로그. **IQ Dev Lab**, **IQ AI Lab**, **IQ Agent Lab**의 탐구 과정을 정제해 정리(Theorem)와 증명(Proof)으로 발행하는 허브.

[![IQ Lab Blog](https://img.shields.io/badge/Visit_Site-00d9ff?style=for-the-badge&logo=astro&logoColor=white)](https://iq-proof.github.io)

</details>

<details>
<summary>&nbsp;<img src="https://api.iconify.design/lucide/server-cog.svg?color=%23ff4d4d" width="16" align="center"/> &nbsp;<b>IQ Dev Lab</b> &nbsp;—&nbsp; Java 백엔드 본질 탐구 연구소</summary>

<br/>

> ***"Beyond the docs — into the essence of Java backend."***

공식 문서를 넘어 Java 백엔드의 본질을 탐구.

Java · Spring · Architecture · Infrastructure · Database — 백엔드 전 영역의 내부 메커니즘을 "**원리 → 트레이드오프 → 설계 → 구현 → 검증**"의 흐름으로 "**왜 그렇게 작동하는가**"를 끝까지 파고드는 Java 백엔드 연구소.

📝 정제된 결과물은 [**IQ Lab Blog**](https://iq-proof.github.io)에 발행됩니다.

[![IQ Dev Lab](https://img.shields.io/badge/Visit_Site-ff4d4d?style=for-the-badge&logo=gitbook&logoColor=white)](https://iq-dev-lab.github.io)
[![Organization](https://img.shields.io/badge/GitHub_Org-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/iq-dev-lab)

</details>

<details>
<summary>&nbsp;<img src="https://api.iconify.design/lucide/brain-circuit.svg?color=%23a78bfa" width="16" align="center"/> &nbsp;<b>IQ AI Lab</b> &nbsp;—&nbsp; AI의 수학적 본질을 증명하는 연구소</summary>

<br/>

> ***"Prove, don't memorize — the math behind AI."***

수학적 증명으로 AI의 본질을 탐구.

RL·DL·NLP·LLM의 핵심 논문과 수학적 유도 과정을 AI와 함께 분석하고, "**가정 → 보조정리 → 증명 → 구현 → 통찰**"의 흐름으로 알고리즘의 "**왜 작동하는가**"를 끝까지 파고드는 AI 연구소.

📝 정제된 결과물은 [**IQ Lab Blog**](https://iq-proof.github.io)에 발행됩니다.

[![IQ AI Lab](https://img.shields.io/badge/Visit_Site-a78bfa?style=for-the-badge&logo=openai&logoColor=white)](https://iq-ai-lab.github.io)
[![Organization](https://img.shields.io/badge/GitHub_Org-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/iq-ai-lab)

</details>

<details>
<summary>&nbsp;<img src="https://api.iconify.design/lucide/bot.svg?color=%23cc785c" width="16" align="center"/> &nbsp;<b>IQ Agent Lab</b> &nbsp;—&nbsp; 에이전트 시스템·자동화 인프라 연구소</summary>

<br/>

> ***"인간이 방향을 정하고, 에이전트가 양산하고, 큐레이션이 품질을 보증한다."***

에이전트 시스템으로 창작 영역을 자동화하는 1인 연구소.

도메인별 자동화 도구(iq-blogger · iq-label · iq-writer · iq-painter · iq-studio · iq-curator)를 만들고, 각 도구의 첫 검증 사례를 운영합니다. **iq-blogger**는 첫 도구로, [iq-proof](https://iq-proof.github.io) 블로그를 자동 생산하는 중.

📝 운영 메트릭과 시스템 회고는 [**IQ Lab Blog**](https://iq-proof.github.io)에 발행됩니다.

[![IQ Agent Lab](https://img.shields.io/badge/Visit_Site-cc785c?style=for-the-badge&logo=anthropic&logoColor=white)](https://iq-agent-lab.github.io)
[![Organization](https://img.shields.io/badge/GitHub_Org-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/iq-agent-lab)

</details>

<details>
<summary>&nbsp;<img src="https://api.iconify.design/lucide/orbit.svg?color=%2300bfa5" width="16" align="center"/> &nbsp;<b>Woowacourse 8th BE — IQ Archive</b> &nbsp;—&nbsp; 우테코 8기 백엔드 아카이브</summary>

<br/>

> ***"I Question, therefore I am."***

우아한테크코스 8기 백엔드 크루로 참여하며, 표면적인 구현을 넘어 끊임없는 '**질문(Question)**'을 통해 객체 지향과 자바의 본질을 탐구하는 기록소.

[![Woowa 8th BE IQ](https://img.shields.io/badge/Visit_Archive-00bfa5?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Woowa-8th-BE-IQ)

</details>

<details>
<summary>&nbsp;<img src="https://api.iconify.design/lucide/youtube.svg?color=%23FF0000" width="16" align="center"/> &nbsp;<b>See Why</b> &nbsp;—&nbsp; 수학·AI 시각화 유튜브 채널</summary>

<br/>

> ***"Complex concepts visualized — so the answer feels inevitable, not memorized."***

수학·AI·알고리즘의 핵심 **직관**을 manim으로 **시각화**하고, 보고 나면 당연하게 느껴지도록 풀어내는 채널.

[![See Why](https://img.shields.io/badge/Visit_Channel-FF0000?style=for-the-badge&logo=youtube&logoColor=white)](https://www.youtube.com/@SeeWhy-l3n)

</details>

<br/>

## <img src="https://api.iconify.design/lucide/feather.svg?color=%231a1a1a" width="28"/> Saga Tales

<div align="center">

> ***"우리의 사가를, 하나의 이야기씩."***

<br/>

<a href="https://github.com/Saga-Tales">
  <img src="https://api.iconify.design/lucide/library.svg?color=%238b3a3a" width="32"/>
  <br/>
  <b>Saga Tales</b>
</a>
<br/>
<sub>창업 venture studio</sub>
<br/>
<br/>
<a href="https://github.com/Saga-Tales">
  <img src="https://img.shields.io/badge/Organization-8b3a3a?style=flat-square&logo=github&logoColor=white" />
</a>

</div>

<br/>

창업을 준비하며 매주 하나의 Tale을 만들고 검증합니다.

### Tales

<details>
<summary>&nbsp;<b>Tale 01 · diary</b> &nbsp;—&nbsp; 사적인 일기 에이전트</summary>
<br/>

소중한 사람들에 대한 일상을 기록하면 LLM이 자동으로 사실(fact)을 누적하고, 의미 검색으로 과거를 회상해 맥락 있는 조언을 줍니다.
데이터와 API 키 모두 사용자 브라우저에만 살고, 백엔드 서버는 존재하지 않습니다.

[![Live Site](https://img.shields.io/badge/Live_Site-d4a574?style=for-the-badge&logo=githubpages&logoColor=white)](https://saga-tales.github.io/tale-01-personal-diary-iq/)
[![Repo](https://img.shields.io/badge/Repo-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Saga-Tales/tale-01-personal-diary-iq)

</details>

<details>
<summary>&nbsp;<b>Tale 02 · convo·trace</b> &nbsp;—&nbsp; 영어 회화 학습 도구</summary>
<br/>

영어 회화 도중 만나는 모든 학습 모먼트(짧은 답변·모르는 단어·좋은 표현)를 컨텍스트와 함께 흔적으로 남기고, 남긴 표현이 다음 회화의 시나리오에 자연스럽게 다시 등장하는 회상 폐쇄 루프 학습 도구.
N명 페어 모드와 QR 캡처 동기화로 영어 회화 동아리 페어 활동에서도 학습이 누적됩니다.
데이터와 API 키 모두 사용자 브라우저에만 살고, 백엔드 서버는 존재하지 않습니다.

[![Live Site](https://img.shields.io/badge/Live_Site-d4a574?style=for-the-badge&logo=githubpages&logoColor=white)](https://saga-tales.github.io/tale-02-convo-trace-iq/)
[![Repo](https://img.shields.io/badge/Repo-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Saga-Tales/tale-02-convo-trace-iq)

</details>

<br/>

## <img src="https://api.iconify.design/lucide/trophy.svg?color=%231a1a1a" width="28"/> Achievements

<details>
<summary>&nbsp;<b>Baekjoon Online Judge</b></summary>

<br/>

<a href="https://solved.ac/ibm514">
  <img src="http://mazassumnida.wtf/api/v2/generate_badge?boj=ibm514&theme=dark" height="170" alt="Solved.ac Tier" />
</a>

</details>

<details>
<summary>&nbsp;<b>Collegiate Programming Contest</b></summary>

<br/>

**Finalist**, SCPC (Samsung Collegiate Programming Cup) **[Algorithm Track]**
[🔗 대회 정보 보러가기](https://raw.githubusercontent.com/e9ua1/e9ua1/main/cpc/SCPC_2025_Poster.jpg)

**Finalist**, ACPC (AWS × Codetree Programming Contest)
[🔗 대회 정보 보러가기](https://raw.githubusercontent.com/e9ua1/e9ua1/main/cpc/ACPC_2025_Poster.png)

</details>

<details>
<summary>&nbsp;<b>Certifications</b></summary>

<br/>

| 자격증명 | 영문명 | 취득일 | 수준 | 발급 기관 |
| :--- | :--- | :---: | :---: | :--- |
| **정보처리기사** | Engineer Information Processing | 2024-09-10 | - | 한국산업인력공단 |
| **빅데이터분석기사** | Engineer Big Data Analysis | 2025-07-11 | - | 한국데이터산업진흥원 |
| **SQLD** | SQL Developer | 2024-09-20 | - | 한국데이터산업진흥원 |
| **ADsP** | Data Analytics Semi-Professional | 2024-11-29 | - | 한국데이터산업진흥원 |
| **TOPCIT** | Test of Practical Competency in IT | 2025-05-24 | 3 | 정보통신기획평가원 |
| **PCCP [C++]** | Certified Coding Professional [C++] | 2025-02-21 | Lv.2 | 프로그래머스 |

</details>
